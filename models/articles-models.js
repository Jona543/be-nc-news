...
const fetchArticleById = (article_id, topics) => {
  return db
    .query(
      "SELECT articles.*, COUNT(comments.comment_id) AS comment_count FROM articles LEFT OUTER JOIN comments ON articles.article_id = comments.article_id WHERE articles.article_id = $1 GROUP BY articles.article_id;",
      [article_id]
    )
    .then(({ rows }) => {
      if (rows.length === 0) {
...
const fetchArticles = (userQuery, topics) => {
  ...
  if (topic) {
    const allTopics = [];
    topics.rows.forEach((item) => {
      if (item.slug === topic) {
        allTopics.push(item.slug);
      }
    });
...
const insertComment = (article_id, username, body, users) => {
  const names = [];
  users.forEach((user) => {
